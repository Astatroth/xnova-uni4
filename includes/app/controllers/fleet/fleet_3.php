...
if (($HeDBRec['authlevel'] > 0 && user::get()->data['authlevel'] == 0) && ($fleetmission != 4 && $fleetmission != 3))
...
$flyingFleets = db::first(db::query("SELECT COUNT(fleet_id) as Number FROM game_fleets WHERE `fleet_owner`='".user::get()->data['id']."'", true));
...
if ($fleetmission != 15)
{
	...
	if ($fleetmission == 5)
	{
		...
		if ($HeDBRec['ally_id'] != user::get()->data['ally_id'] && !isset($friend['id']) && (!isset($ad['id']) || (isset($ad['id']) && $ad['type'] != 2)))
			$this->message("<span class=\"error\"><b>Нельзя охранять вражеские планеты!</b></span>", 'Ошибка', "?set=fleet", 2);
	}

	if ($TargetPlanet['id_owner'] == user::get()->data['id'] && ($fleetmission == 1 || $fleetmission == 2))
...
$StockDeuterium = app::$planetrow->data['deuterium'] - ($consumption + $TotalFleetCons);
...
if ($fleetmission == 1)
{
	...
	if (!user::get()->isAdmin() && isset($log['kolvo']) && $log['kolvo'] > 2 && ((isset($ad['id']) && $ad['type'] != 3) || !isset($ad['id'])))
...
if ($fleet_group_mr > 0 && $i > 0 && $fleet_group_time > 0)
{
	foreach ($arrr AS $id => $row)
	{
		...
		db::query("UPDATE game_fleets SET fleet_start_time = " . $fleet_group_time . ", fleet_end_time = " . $end . ", fleet_time = " . $fleet_group_time . " WHERE fleet_id = '" . $row['id'] . "'");
	}
}

if (($fleetmission == 1 || $fleetmission == 2 || $fleetmission == 3) && $HeDBRec['id'] != user::get()->data['id'] && !user::get()->isAdmin())
{
...
if ($fleetmission == 3 && $HeDBRec['id'] != user::get()->data['id'] && !user::get()->isAdmin())
{
	...
	(
		...
		'data' 		=> "s:[".app::$planetrow->data['galaxy'].":".app::$planetrow->data['system'].":".app::$planetrow->data['planet']."(".app::$planetrow->data['planet_type'].")];e:[".$galaxy.":".$system.":".$planet."(".$planettype.")];f:[".$fleet_array."];m:".$TransMetal.";c:".$TransCrystal.";d:".$TransDeuterium.";",
...
$tutorial = db::query("SELECT id, quest_id FROM game_users_quests WHERE user_id = ".user::get()->getId()." AND finish = '0' AND stage = 0", true);
...
$fleetPlanetUpdate['metal'] 	= app::$planetrow->data["metal"];
$fleetPlanetUpdate['crystal'] 	= app::$planetrow->data["crystal"];
$fleetPlanetUpdate['deuterium'] = app::$planetrow->data["deuterium"];

sql::build()->update('game_planets')->set($fleetPlanetUpdate)->where('id', '=', app::$planetrow->data['id'])->execute();
