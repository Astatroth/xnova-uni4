...
if (!isset($_POST['send']))
{
	if (isset($_POST['action']) && $_POST['action'] == 'addaks')
	{
		if (empty($fleet['fleet_group']))
		{
			...
			{
				$allyMembers = db::query("SELECT u_id FROM game_alliance_members WHERE a_id = ".user::get()->data['ally_id']."");

				while ($member = db::fetch($allyMembers))
				{
					db::query("INSERT INTO game_aks_user VALUES (" . $aks['id'] . ", " . $member['u_id'] . ")");
				}
			}*/
	...
	elseif (isset($_POST['action']) && $_POST['action'] == 'adduser')
	{
		...
		{
			$user_data = db::query("SELECT * FROM game_users WHERE id = '" . intval($_POST['userid']) . "'", true);
		}
		else
		{
			$user_data = db::query("SELECT * FROM game_users WHERE username = '" . db::escape_string($_POST['addtogroup']) . "'", true);
		}
	...
	elseif ($fleetid == $aks['fleet_id'])
	{
		...
		$buddies = db::query("SELECT u.id, u.username FROM game_buddy b, game_users u WHERE u.id = b.sender AND b.owner = ".user::get()->getId()." AND active = '1'");
